...
void Lyra3CoversTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (hasContinueReading) {
    ...
    for (int i = 0; i < std::min(static_cast<int>(recentBooks.size()), Lyra3CoversMetrics::values.homeRecentBooksCount);
      ...
      // ChineseType 移植：原 wrappedText (多行) 改成單行 truncatedText
      std::vector<std::string> titleLines;
      titleLines.push_back(renderer.truncatedText(SMALL_FONT_ID, recentBooks[i].title.c_str(), maxLineWidth));
    ...
    // ChineseType 移植：drawEmptyRecents helper 在 Carousel upstream 還沒實作 — 留空
    (void)rect;
  }
}
